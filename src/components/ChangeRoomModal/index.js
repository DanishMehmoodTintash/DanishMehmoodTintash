...
const ChangeRoomModal = ({
  ...
  const selectCuratedRoom = () => {
    ...
    track("Take Me to Curated Room ", {
      [trackingProps.ROOM_NAME]: currentCollection.get("type_name"),
      [trackingProps.CURATED_ROOM_NAME]:
        currentCuratedRoom.get("curated_room_name"),
    });
  };
      ...
      {isChangeRoomOpen && (
        <>
          ...
          <EscapeOutside onEscapeOutside={onClose}>
            ...
            >
              <div
                className={`${styles["reset-popup"]} ${styles["curated-popup"]}`}
              >
                <div
                ...
                <div className={styles["reset-desc"]}>
                  Changing your dorm's vibe will override all of your product
                  selections from your dorm and shopping list.
                </div>
                <button
                  className={`${styles["reset-popup-btn"]} ${styles["curated-room-popup-btn"]}`}
                  onClick={selectCuratedRoom}
                >
                  Yes, Take Me To a New Vibe
                </button>
                <button
